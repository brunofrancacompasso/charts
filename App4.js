...
const Container = styled.ScrollView.attrs(() => ({
  contentContainerStyle: {paddingBottom: 64, paddingTop: 32},
}))`
  width: 100%;
  flex: 1;
  padding: 24px;
...
const data2 = new Array(500)
  .fill(undefined)
  .map((v, i) => ({x: i / 10, y: Math.random() * 10 + 5}));
...
const App = () => {
  ...
  return (
    <Container>
      ...
      <TouchableOpacity
        onPress={() => {
          ref?.current?.setTooltipIndex(1);
          ref3?.current?.setTooltipIndex(2);
        }}>
        <Text>Set tooltip to second datapoint</Text>
      </TouchableOpacity>
      <TouchableOpacity
        onPress={() => ref?.current?.setTooltipIndex(undefined)}>
        <Text>Hide tooltip</Text>
      </TouchableOpacity>
      <TouchableOpacity
        onPress={() => ref2?.current?.setViewportOrigin({x: 0, y: 0})}>
        <Text>Set viewport to start</Text>
      </TouchableOpacity>
      <TouchableOpacity
        onPress={() => ref2?.current?.setViewportOrigin({x: 5, y: 0})}>
